...
STATUS_WRITER="$SCRIPT_DIR/status_writer.sh"

if [[ -f "$STATUS_WRITER" ]]; then
    # shellcheck source=/dev/null
    source "$STATUS_WRITER"
fi
...
update_status() {
    ...
    if declare -F write_status >/dev/null 2>&1; then
        write_status "$status" "$current_video" "$playlist_index" "$playlist_total" "$mode" "$audio" "$video_bitrate" "$maxrate" "$bufsize" "$last_error"
    fi
}
...
load_videos() {
    ...
    mapfile -t raw_videos < <(find -L "$VIDEO_DIR" -maxdepth "$FOLDER" -type f \( "${video_find_args[@]}" \) 2>/dev/null)
...
while true; do
    ...
    update_status "starting" "$base" "$((idx + 1))" "$TOTAL" "checking" "$audio" "-" "-" "-" ""
    ...
    if [[ "$copy_selected" == "yes" ]]; then
        ...
        update_status "playing" "$base" "$((idx + 1))" "$TOTAL" "transcode" "$audio" "$VIDEO_BITRATE" "$MAXRATE" "$VIDEO_BUFSIZE" ""
